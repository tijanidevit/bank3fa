...
<script>
    ...
    $('#transferForm').submit((e) =>{
        ...
        if (transferOtp != $('#otp').val()) {
            $('#otpResponse').html(`<p class="text-danger">Invalid transfer OTP</p>`)
            return
        }

        if ('{{ auth()->user()->question->answer }}' != $('#answer').val()) {
            console.log('hhhh', '{{ auth()->user()->question->answer }}')
            $('#answerResponse').html(`<p class="text-danger">Provided answer is incorrect</p>`)
            return
        }
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $('#response').append(`<img src="{{ asset('assets/success.gif')  }}"/>`)
                $('#amount').val("");
                $('#accountNumber').val("")
                $('#otp').val("")
                $('#answer').val("")
