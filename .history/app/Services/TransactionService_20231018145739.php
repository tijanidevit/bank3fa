...
use App\Notifications\TransferOtpNotification;
...
class TransactionService
{
    ...
    {
        $user = auth()->user();
        $otp = $user->otp()->create(['otp' => rand(111111,999999), 'type' => OTPType::TRANSFER]);
        $user->notify(new TransferOtpNotification($otp->otp));
        return $otp;
    }
    ...
    protected function saveTransaction($user,$amount,$remark,$type)
    {
        DB::transaction(function () use ($user,$amount,$remark,$type)
        {
            ...
            $user->notify(new TransferNotification(str_replace('&#8358;','NGN',$remark), $amount));
        });
    }
